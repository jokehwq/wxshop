...
public class ClientCustomSSL {
	...
	private static String partner = "1520196941";
	//p12证书的位置
    ...
    public static String doRefund(String url,String data, String p12name, String par) throws Exception {
        KeyStore keyStore  = KeyStore.getInstance("PKCS12");
        URL url2 = ClientCustomSSL.class.getClassLoader().getResource(p12name);
        URI uri = url2.toURI();
        FileInputStream instream = new FileInputStream(new File(uri));//P12文件目录
        try {
            keyStore.load(instream, par.toCharArray());
        } finally {
            instream.close();
        }
        SSLContext sslcontext = SSLContexts.custom()
                .loadKeyMaterial(keyStore, par.toCharArray())//这里也是写密码的
                .build();
