...
<body id="wrap">
    ...
    <dl class="drdd-info6" onclick="window.location.href='addrList.html?cps_id=${cps_id}&addr_id=${addr_id}'">
     ...
     <input type="hidden" value="${addr.addr_id }" id='addr_id_two'>
    ...
    <p>收货地址：<span id="province" lang="${addr.addr }">${addr.addr }</span>
        	${addr.addr_name }</p>
        </dt>
       </c:forEach> 
    ...
    <button class="drdd-btn" onclick="add()" >确认下单</button>
    <script type="text/javascript">
    function fgt(){
    	var wuliu ='${fgt_price}';
    ...
    function add(){
		...
		var note= $('#note').val();
    	 addr_name=addr_user+' '+addr_tel+' '+province;
    	$.ajax({
			...
			+'&addr_id='+$('#addr_id_two').val()
			+'&receive='+encodeURI(receive)+'&note='+encodeURI(note),
			success:function(rs){
				var rrs = rs.order_id;
				var re = /^[0-9]+.?[0-9]*$/;    
				if(re.test(rrs)&&rrs!=0){
					
					window.location.href='payOrder.html?order_id='+rrs;
				}else{
					alert("失败！");
				}
			}
