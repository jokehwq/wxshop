...
<body>
    ...
    <div class="drdd-info2">
        ...
        <button class="drdd-btn" onclick="callpay()">微信支付</button>
          ...
          <c:if test="${list.goods_total>1}">
        <button class="drdd-btn" onclick="callpay1('${list.order_id}')">货到付款</button>
        </c:if>
        </c:forEach>
    </div>
    	<script type="text/javascript">
  	function callpay(){
  		  alert('暂未开通');return false; 
  	...
  	/* 货到付款 */
  	 function callpay1(order_id) {
         $.ajax({
         //几个参数需要注意一下
             type: "POST",//方法类型
             dataType: "json",//预期服务器返回的数据类型
         	url:'orderUpdate.html',//调用的路径
			data:'order_id='+order_id+'&status=1',//参数
			success:function(rs){//状态
 				if(rs==1){
 					alert("成功！");
 					 window.location.href='orderList.html';
 				}else{
