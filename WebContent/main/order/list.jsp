...
<body style="/* width: 1720px */">
				...
				<tbody>
					...
					<c:set value="ord${s.index}" var="ord"></c:set>
						<tr>
							...
							${list.cps_name}到付</c:if></td>
							<td class="text-c"<%--  id="tprice${s.index}" --%>>
							<input type="text"  <c:if test="${list.status==0 }">onblur="tp('${list.order_id}','total${s.index}')" </c:if>
							<c:if test="${list.status==5 }">onblur="tp('${list.order_id}','total${s.index}')" </c:if>
							<c:if test="${list.status!=0 }"> readonly="readonly" </c:if>
							 id="total${s.index}" class="input-text" value="${list.goods_total}" style="border: 0px solid #fff">
							...
							<c:if test="${list.status==-5 }"><a href="javascript:;" onclick="refund('${list.order_id}','${list.goods_total }')">退款</a><br></c:if>
	...
	function refund(order_id, goods_total){
		/* alert(goods_total);return; */
		var  b = confirm('确定退款吗？');
		if(!b){
			return ;
		}
		$.ajax({
			...
			type:'post',
			data:'order_id='+order_id+'&status=-6'+'&goods_total='+goods_total+'&netnum='+netnum,
			beforeSend: function(){
				layer.msg('退款中', {
				    icon: 16
				    ,shade: 0.01
		...
		if (currentPage > 1) {
			window.location.href = 'orderList.html?currentPage='
					+ (parseInt(currentPage) - 1)+'&status='+status+'&start_time=' + encodeURIComponent(encodeURIComponent(start_time))
						+ '&end_time=' + encodeURIComponent(encodeURIComponent(end_time))+ '&ctg_name=' + encodeURIComponent(encodeURIComponent(ctg_name))
						+ '&goods_name=' + encodeURIComponent(encodeURIComponent(goods_name))+ '&addr_name=' + encodeURIComponent(encodeURIComponent(addr_name))+'&netnum='+'${netnum }';
			return;
		}
	}
	function downPage() {
