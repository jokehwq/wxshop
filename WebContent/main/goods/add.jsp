...
<script>
			...
			/* 	console.log(111);
				editor.appendHtml('dsadas<a>adasd</a>') */
			});
		</script>
<title>基本设置</title>
...
<body>
	...
	<script type="text/javascript" src="js/aliyunoss.js"></script>
	...
	function add(){
		...
		$.ajax({
			...
			+'&goods_price='+goods_price+'&goods_detail='+goods_detail+'&netnum='+netnum,
			success:function(rs){
				if(rs==1){
	...
	//上传图片	
	  document.getElementById('file').addEventListener('change', function (e) {
	  		...
	  		client.multipartUpload(storeAs, file).then(function (result) {
	  			...
	  			$('#img').append("<img src='"+imgName+"' width='100' height='100'>");
	  			alert("上传成功");
	  			console.log(imgName);
	  	 	}).catch(function (err) {
	  	 		alert("上传失败,请重试");
	 ...
	 function alertmsg(){
		 layer.open({
			 ...
			 title: '上传图片',
			 area: ['430px', '260px'],
			 content: 'goods/uploadImg.jsp'
		 });
  function  goods_price(goods_price) {
			    var source = String(goods_price).split(".");//按小数点分成2部分
